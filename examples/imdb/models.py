...
from indrnn import indrnn, pirnn, dtirnn
...
class PIRNN(ojnn.Module):
    ...
    def __init__(
        ...
        _assert_negative_axis(seq_dim)
        _assert_negative_axis(channel_dim)
        self.assign_(seq_dim=seq_dim, channel_dim=channel_dim, unroll=unroll)
...
class DTIRNN(ojnn.Module):
    seq_dim: int
    channel_dim: int
    unroll: bool | int
    weight_1: Array = ojnn.parameter()
    weight_2: Array = ojnn.parameter()
    ...
    def reset(self, input_shape, _=None):
        weight_1 = ojnn.ones([input_shape[self.channel_dim]])
        weight_2 = ojnn.zeros([input_shape[self.channel_dim]])
        return ojnn.new(
            self, weight_1=weight_1, weight_2=weight_2
        ), tuple(input_shape)
...
class PRNNStep(ojnn.RecStep):
    out_features: int = ojnn.config()
    dtype: DTypeLike | None = ojnn.config()
    weight: Array = ojnn.parameter()
    state_h: Array = ojnn.buffer()
    ...
    def reset(self, input_shape, _=None):
        ...
        weight_hh = ojnn.eye(out_features, out_features, dtype=dtype)
        return ojnn.new(self, weight=weight_hh), output_shape

    @property
    def reset_state_rngkey_count(self) -> int:
        return 0

    def reset_recurrent_state(self, batch_shape, _=None):
        dtype = self.dtype
        state_h = ojnn.zeros([*batch_shape, self.out_features], dtype=dtype)
        return ojnn.new(
            self, state_h=state_h, recurrent_state_initialized=True
        )

    def unset_recurrent_state(self: Self) -> Self:
        return ojnn.new(
            self,
            state_h=ojnn.ftypes.make_default_array(),
    ...
    def forward(self, x, _=None):
        output = x + ojnn.dense(ojnn.tanh(self.state_h), self.weight)
...
class PIRNNNet(ojnn.Sequential):
    def __init__(
        self,
        depth: int = 4,
        vocab_size: int = 400001,
        ...
        super().__init__(
            ...
            ojnn.Dropout1d(drop_rate),
            *rec_layers,
            # ojnn.GlobalPool("max", (-2,), keepdims=False),  # 0.8960
            ojnn.GlobalPool("avg", (-2,), keepdims=False),  # 0.9043
            # TakeLast(-2),  # 0.7646
            # BC
            HeInitDense(2),
            # B2
...
class DTIRNNNet(ojnn.Sequential):
    def __init__(
        self,
        depth: int = 4,
        vocab_size: int = 400001,
        ...
        super().__init__(
            ...
            ojnn.Dropout1d(drop_rate),
            *rec_layers,
            ojnn.GlobalPool("avg", (-2,), keepdims=False),
            # BC
            HeInitDense(2),
            # B2
        )
...
class PRNNNet(ojnn.Sequential):
    def __init__(
        ...
        rec_step = ojnn.SequentialRecStep(*rec_step_layers)
        ...
        super().__init__(
            ...
            # BTC
            ojnn.Dropout1d(drop_rate),
            ojnn.Recurrent(rec_step),
            # TakeLast(-2),
            ojnn.GlobalPool("max", (-2,), keepdims=False),
            # BC
            HeInitDense(2),
            # B2
        )
